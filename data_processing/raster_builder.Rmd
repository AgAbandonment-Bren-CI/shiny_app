...
bio_25k <- aggregate(bio, fact = 5, fun = 'mean', na.rm = TRUE) %>% 
    project(y = crs(ssp1_50k))

# export raster
writeRaster(bio_25k, 
            here('data','processed','global','biodiversity_global_25km.tif'),
            overwrite = TRUE)
```
...
# lower resolution to 25km
carbon_25k <- aggregate(carbon, fact = 25, fun = 'mean', na.rm = TRUE) %>% 
  project(y = crs(ssp1_50k))

# export raster
writeRaster(carbon_25k, 
            here('data','processed','global','carbon_global_25km.tif'),
            overwrite = TRUE)
```
